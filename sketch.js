...
function draw() {
 ...
 shootGroup.overlap(enemieGroup, function(shoot, enemie){
   var roll = Math.round(random(0,50));
   score += 25;
   console.log(roll);
   if(roll !== 1){
     shoot.destroy(); 
 ...
 power_up_Group.overlap(player, function(power_up, player){
  power_up.destroy();
  timer4 = 0;
  powerBar = true;
  if(timer4 < 500){
 ...
 textSize(20);
 text(Math.round(time/20), 900, 100);
...
function Power(a,b,ax,bx){    
  power_up = createSprite(a,b, 35, 35); 
  power_up.setVelocity(ax,bx);
  power_up.shapeColor = "lightBlue";
